...
def get_output_slices(
    output_epsg: int,
    bounds: Bbox | tuple[float, float, float, float] | None = None,
    ...
    ------
    ValueError
        If neither bounds nor aoi_wkt, or both bounds and aoi_wkt, is passed.

    """
    from pyproj import CRS, Transformer
    from shapely import from_wkt, geometry, ops

    if bounds is None and aoi_wkt is None:
        msg = "Must pass either `bounds` or `aoi_wkt`"
        raise ValueError(msg)
    if bounds is not None and aoi_wkt is not None:
        msg = "Cannot pass both `bounds` and `aoi_wkt`"
        raise ValueError(msg)

    # Create a polygon from the bounds or wkt
    bounds_poly = geometry.box(*bounds) if bounds is not None else from_wkt(aoi_wkt)
    ...
    if input_epsg != output_epsg:
        ...
        transformer = Transformer.from_crs(
            CRS.from_epsg(input_epsg), output_epsg, always_xy=True
        )
        output_polygon = ops.transform(transformer.transform, bounds_poly)
    else:
        output_polygon = bounds_poly
    left, bottom, right, top = output_polygon.bounds
    # top-> bottom, left->right are slice orderings
