...
# NISAR data is hosted at ASF's NISAR DAAC
NISAR_EARTHDATA_HOST = "urs.earthdata.nasa.gov"
...
def open_file(
    ...
    """Open a remote (or local) NISAR GSLC HDF5 file using fsspec.
    ...
    if parsed.scheme in {"http", "https"}:
        fs = get_https_fs(earthdata_username, earthdata_password)
...
def open_h5(
    ...
    """Open a remote (or local) NISAR GSLC HDF5 file.
...
def get_s3_fs() -> s3fs.S3FileSystem:
    """Create an fsspec filesystem object authenticated using AWS credentials.
